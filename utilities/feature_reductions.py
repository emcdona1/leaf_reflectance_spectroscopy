...
def pca(X_train, y_train, X_test, y_test, n_features=10) \
    ...
    return X_transformed_train, y_train, X_transformed_test, y_test
...
def anova(X_train, y_train, X_test, y_test, n_features='all') -> \
...
def mutual_info(X_train, y_train, X_test, y_test, n_features='all') \
        -> (np.ndarray, np.ndarray, np.ndarray, np.ndarray, SelectKBest):
